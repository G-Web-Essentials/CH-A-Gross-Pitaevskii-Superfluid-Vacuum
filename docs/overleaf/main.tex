...
\begin{enumerate}[label=\textbf{Step \arabic*:}, leftmargin=*]
  ...
  Turn-on gap $d_c$ is the immediate laboratory output; identifying $\xi$ additionally requires ripple period $a_{\mathrm{vac}}$ from Prediction~\#3.
\end{enumerate}
...
The steps below document how laboratory outputs \emph{could} be mapped to $\xi$ and then to emergent-gravity diagnostics if---and only if---real data support the identification hypotheses; they are not part of the Tier~A pass/fail decision.
...
The order below runs from the completed GRB reanalysis through GPE forecasts, the protocol demo, and the power study---the same path used to verify internal consistency before outreach.
