...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\rho_{\mathrm{in}}$ scale & $\sim 10^{19}$--$10^{24}\,\mathrm{kg\,m^{-3}}$ depending on $\xi$---\textbf{far denser than neutron stars} \\
...
\begin{equation}
  ds^2 = \frac{\rho_{\mathrm{in}}}{c_s}\left[
    -(c_s^2 - |\mathbf{v}|^2)\,dt^2 - 2\,\mathbf{v}\cdot d\mathbf{x}\,dt + |d\mathbf{x}|^2
  \right].
\end{equation}
The $-(c_s^2 - |\mathbf{v}|^2)$ coefficient has the same $(1 - v^2/c^2)$ structure as Minkowski time when $c_s$ is identified with $c$; proper time along a fast worldline is shorter than coordinate time.
...
\textbf{Standard view:} $\Lambda$ or dark energy with $\rho_\Lambda \sim 10^{-26}\,\mathrm{kg\,m^{-3}}$.
