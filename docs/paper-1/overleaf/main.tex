% Chronos-Hydrodynamics — Paper 1 --- submission build (main.tex)
% Trimmed for arXiv / Casimir outreach: no gravity-route appendix (see gravity-sketches.tex).
...
% Breakable monospaced script names and URLs (Overleaf/pdfLaTeX line overflow)
\newcommand{\py}[1]{\nolinkurl{#1}}
...
\maketitle
\begin{center}
  \small Zenodo preprint (v1): \url{https://doi.org/10.5281/zenodo.21296947}
\end{center}
\vspace{0.75em}
...
Companion Paper~2~\cite{CHPaper2} (LaTeX source on GitHub; preprint in preparation) derives gradient gating in a perturbative EM--GPE bridge and favours $\alpha_{\max} \sim 0.1$--$0.2\%$ from grain microphysics; the $12\%$ and $\kappa=0.12$ defaults here remain upper sensitivity cases for the power study.
\end{abstract}
...
Section~\ref{sec:theory} supplies the equations; here we state the claims and their role in the program.

\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*]
  \item \textbf{Ontological:} Space is a complex order parameter $\psi(\mathbf{x},t)$ in a macroscopic quantum state (BEC).
  Without a physical substrate, ``empty'' space has no local state to stress in the laboratory; CH posits a material vacuum rather than a passive stage.

  \item \textbf{Dynamical:} $\psi$ obeys the GPE; Madelung variables $(\rho, S)$ are fundamental hydrodynamic fields.
  The substrate must evolve causally and locally, so density $\rho$ and phase $S$---not separate spacetime and matter fields---carry the dynamics.

  \item \textbf{Matter:} Mass is a \textbf{localized defect} (depletion) in $\rho$, not an object in passive space.
  If space is the fluid, matter cannot be placed inside it from outside; it must be stable structure \emph{in} $\psi$ (a fermion-level derivation remains open; Section~\ref{sec:open-problems}).

  \item \textbf{Invisibility:} Uniform $\rho$ with $|\nabla\rho| \approx 0$ yields \textbf{no observable structure}.
  Decades of null precision tests in quiet vacuum are therefore expected, not failures of the framework---uniform regions must be quiet before gated turn-on can be claimed elsewhere.

  \item \textbf{Gradient gating:} Large $|\nabla\rho|$ drives supersolid order; observables scale with $\chi(|\nabla\rho|)$.
  Casimir ripple, visibility wobble, and gated dispersion become the laboratory discriminant: they turn on only where boundary stress raises $|\nabla\rho|$ past $|\nabla\rho|_c$ (\hyperref[sec:protocol]{Prediction~\#7}).
  ...
  \end{itemize}
  A single substrate cannot borrow $G$, $c$, and time as independent inputs; they must be \emph{derived or matched} from vacuum parameters.
  This is a \textbf{consistency requirement on the CH ontology}, not a stand-alone empirical claim---standard physics uses $G$, $c$, and metric time as separate inputs because it does not posit a single material vacuum.
  ...
  Observers built from the same condensate should recover an effective Lorentzian causal structure at low energy even though the underlying medium is material (analog-gravity program~\cite{Unruh1981,Visser1995}).
\end{enumerate}
...
The falsifiable claim is \textbf{gradient-correlated turn-on} (\hyperref[sec:protocol]{Prediction~\#7}).
This paper walks from postulates through bench observables, GPE knob forecasts, a pre-registered protocol, and the analysis code that implements each step.
...
\begin{itemize}[leftmargin=*]
  ...
  \item A lab-to-theory bridge: measured $k_c$ $\to$ proposed $\xi$ identifications (Section~\ref{sec:xi-lab-route}) and a minimal spherical-defect gravity sketch at grain $\alpha_G^{\mathrm{hydro,ref}}$ (Section~\ref{sec:lab-xi-bridge}).
...
This section turns the postulates above into equations.
A collaborator need not accept the full ontological program to use the protocol below, but every fit model and GPE forecast refers back to these fields and scalings.
We keep the presentation compact: defect gravity, fermions, and full 3D GPE solutions are sketched or deferred (Section~\ref{sec:discussion}).

\subsection{Fundamental equation}

CH describes the vacuum with a single complex field $\psi(\mathbf{x},t)$---the local amplitude and phase of the condensate---rather than with a metric and external matter fields added separately.
...
The healing length $\xi$ is the natural length scale on which $\rho$ recovers from a boundary or defect; $m_{\mathrm{grain}}$ is the inertial mass associated with one phase oscillation at that scale.
Together with bulk density $\rho_{\mathrm{in}}$ and coupling $g$, they fix the sound speed and close the dimensional system:
...
If mass is a defect in $\rho$, the strength of long-range attraction must be expressible in terms of how large a density disturbance the vacuum supports and how stiffly it responds.
CH captures that link in
...
Laboratory and cosmological data probe different aspects of the same $\rho_{\mathrm{in}}$: Casimir and defect work care about inertial response, while cosmic expansion cares about what fraction of that inertia sources curvature.
To address the cosmological-constant problem without abandoning a large $\rho_{\mathrm{in}}$, CH splits inertial and gravitational sourcing:
\begin{equation}
...
Prediction~\#7 does not read $\rho$ directly; it reads observables that CH assumes scale with how sharply $\rho$ varies in space.
$|\nabla\rho|$ is the magnitude of the spatial gradient of $\rho = |\psi|^2$ (Eq.~\ref{eq:madelung}): large near boundaries and defects, $\approx 0$ in uniform void.
...
\textbf{Quantum potential $Q$ vs gradient gate $\chi$:}
The Madelung quantum potential $Q$ (Eq.~\ref{eq:quantum-potential}) is part of the hydrodynamic response of $\rho$ around defects and steep profiles; it is \textbf{not} multiplied by $\chi$.
Gradient gating applies to laboratory \textbf{observables} via Eq.~\ref{eq:gated}, not to $Q$ directly.
Where $|\nabla\rho|$ is large, $|Q|$ typically peaks as well (e.g.\ defect join layers), because both follow from $\rho$ geometry---but whether $Q$ stabilizes or drives moving defect patterns is \textbf{not derived} here.
Kinematic time dilation is sketched through phase $S$ and the acoustic metric (Section~\ref{sec:roadmap}, Layer~3), not through a gated $Q$ channel; full $Q$--defect dynamics belongs in the gravity companion (\texttt{gravity-sketches.tex}) and future work.
...
This section maps GPE fields to the numbers an AFM or interferometer actually returns.
Below we define how $\alpha$, $\Delta V$, and $\beta_{\mathrm{eff}}$ are extracted, which role each prediction plays, and how those scalars enter the knob scan of Prediction~\#7.
The Casimir ripple channel is the minimum collaboration entry point; dispersion is an astrophysical consistency check; the threshold comparison is the discriminating test.
...
Companion Paper~2~\cite{CHPaper2} perturbatively motivates $\alpha_{\mathrm{eff}} \propto \chi_{\mathrm{bulk}}$ and the multiplicative form; Eq.~\ref{eq:casimir-ripple} remains the protocol parameterization here.
...
Dynamic AFM Casimir groups already extract this ripple amplitude; Tier~A collaboration therefore needs only one scalar per gap, not CH-specific hardware.
...
A genuine gradient-gated sector should appear in more than one observable tied to the same $\chi$; requiring $\alpha(k)$ and $\Delta V(k)$ to share $k_c$ (within factor 2) guards against a Casimir-only artifact.
Tier~B adds this channel after Tier~A establishes $\alpha(k)$.
...
GRB timing therefore tests whether an always-on CH extension is already excluded on void paths; it cannot establish gradient gating, which requires raising $|\nabla\rho|$ in the laboratory (Section~\ref{sec:grb-methods}).
...
Isolated nulls in uniform vacuum are compatible with both CH and QFT; the test is whether $O(k)$ \textbf{turns on} across a knob scan that raises $|\nabla\rho|$ while a registered control channel stays flat---a model comparison that can be pre-registered before data collection.
...
\textbf{\#2} tests whether Bell correlators~\cite{Bell1964,Einstein1935} vary with sidereal phase (optional; not central to CH).
\textbf{\#5} tests short-range Yukawa deviations from $1/r^2$ gravity inside cavities (localized gradients only).
CH's gradient-gated design expects global lattice signatures (\#1--\#2) to remain null in quiet laboratories while local boundary tests (\#3, \#6, \#7) may turn on; the present protocol therefore prioritizes the latter group.

\section{Predictions}
\label{sec:predictions}

Table~\ref{tab:predictions} collects all seven forecasts so a reader can see at a glance which tests are null by design in uniform vacuum and which require a stressed boundary.
The laboratory program below implements the primary and discriminating rows; astrophysical and secondary rows bound the framework but do not replace Prediction~\#7.
...
Astrophysical nulls cannot prove CH, but they can exclude the naive version in which $\beta_{\max}$ acts everywhere.
Table~\ref{tab:grb} records that benchmark for GRB~090510 before turning to the laboratory program.
...
Figure~\ref{fig:grb} plots the extended-LAT sample used in that null test.
...
Prediction~\#7 needs a forecast of \emph{which} laboratory knobs raise $|\nabla\rho|$ and \emph{where} in the cavity profile the signal should appear.
Tidal gradients from ordinary masses fall $\sim 10^{-22}$ below $|\nabla\rho|_c$ and cannot serve as the knob; Casimir conductors impose boundary conditions on $\psi$ and can stress the vacuum fluid when the gap $d$ is comparable to the healing length $\xi$.
The GPE boundary program below is a planning tool: it does not replace real $F(d)$ data, but it tells collaborators which scan directions and probe loci matter before hardware time is spent.

\subsection{Thomas--Fermi boundary ansatz}

Full numerical GPE solutions with defect sources remain future work.
For knob planning we use a \textbf{Thomas--Fermi (TF) boundary-layer ansatz} validated against 1D imaginary-time GPE in the companion code~\cite{CHSpaceFabric2026}: $\rho \to 0$ at conductors, $\rho \to \rho_{\mathrm{in}}$ in the bulk, with no claim that TF captures every nm feature of a real sphere--plate geometry.
The profiles read
...
A single $\chi(d)$ curve is ambiguous unless one specifies \emph{where} in the cavity $|\nabla\rho|$ is evaluated.
Ripple amplitude couples to the bulk sector when scanning gap; wall saturation provides the flat control on $d$ and the signal when scanning curvature.
Prediction~\#7 therefore attaches $\chi$ to distinct probe loci (code flag \texttt{coupling=mid} maps bulk $\chi$ to both $\alpha$ and $\Delta V$ in gap demos; \texttt{coupling=wall} is the flat control forecast).
...
Figure~\ref{fig:gpe} shows the resulting wall vs bulk $\chi(d)$ at $\xi = \SI{50}{nm}$ once those probe loci are fixed.
...
Laptop forecasts and the power study need numerical inputs before any collaboration measures $\sigma_\alpha$ or $k_c$.
CH does not fix $\xi$, $\alpha_{\max}$, or per-shot ripple uncertainty from first principles; the values below are \textbf{planning defaults} to be replaced once pilot $F/F_{\mathrm{Cas}}$ fits are registered.

\textbf{Companion Paper 2.}
A separate manuscript~\cite{CHPaper2} (LaTeX source on GitHub; preprint in preparation) couples GPE boundary profiles to a perturbative Casimir mode sum and \textbf{motivates gradient gating} ($\delta F/F_{\mathrm{Cas}} \propto \chi_{\mathrm{bulk}}$ with corr $\approx 0.94$ at $\xi = \SI{50}{nm}$).
Grain-scale EM estimates give a \textbf{preferred} ripple band $\alpha_{\max} \sim 0.1$--$0.2\%$ (classical polarizability, $\kappa \sim 0.01$--$0.02$).
The values $\kappa = 0.12$ and $\alpha_{\max} = 0.12$ used in this paper for power studies and practice fits are \textbf{upper sensitivity cases}, not microphysics-derived predictions.
Paper~2 does \textbf{not} change the Prediction~\#7 protocol here; it tightens priors on amplitude and period before data collection.
...
Measured $k_c$ or $a_{\mathrm{vac}}$ from Eqs.~\ref{eq:casimir-ripple} would supersede it via Section~\ref{sec:xi-lab-route}.
...
The value $0.12$ (12\% on $F/F_{\mathrm{Cas}}$) is chosen in \texttt{casimir\_ripple\_sim.py} as ``visible but still subtle'' for practice fits---order-of-magnitude \textbf{headroom for sensitivity studies}, not a derived prediction (preferred microphysics band $\sim 0.1$--$0.2\%$: Section~\ref{sec:forecast-defaults}, companion Paper~2~\cite{CHPaper2}).
...
With $\xi = \SI{50}{nm}$ and gap scan $d = 40$--$600$~nm ($k = 10^9/d_{[\mathrm{m}]}$), the GPE pipeline (\py{ch_gpe_to_threshold_demo.py}) maps $d \to \chi_{\mathrm{bulk}}$ and $\chi_{\mathrm{wall}}$ and generates synthetic $(k, \alpha, \sigma)$ and $(k, \Delta V, \sigma)$ CSVs.
Flat vs threshold fits (\py{gradient_threshold_analysis.py}) on that synthetic data yield $\Delta\chi^2 \approx 274$ ($\alpha$) and $\Delta\chi^2 \approx 429$ ($\Delta V$)---well above the detection cut $\Delta\chi^2 = 9$.
...
The subsections below separate what to measure, how to decide between models, and what outcomes would support or refute the gradient-gated sector---all specified before data change hands.

\subsection{Measurement channels}

Signal channels ask whether $O(k)$ rises when GPE forecasts $\chi$ rises.
Control channels hold geometry fixed while varying a knob that should \emph{not} track $|\nabla\rho|$ in CH (temperature, duplicate runs), so a threshold in the signal cannot later be attributed to drift or batch error alone.
...
Tidal or bulk-mass gradients cannot approach $|\nabla\rho|_c$ in the laboratory (Section~\ref{sec:gpe}); nm-scale Casimir separations are the practical stress knob.
Gap $d$ is the Tier~A entry point; curvature $R$ enters at Tier~C.

\subsection{Tier A workflow (minimum viable collaboration)}

The path below is for a group with dynamic AFM Casimir capability that can export ripple amplitudes.
No CH-specific hardware is required---only a registered scan plan, a control run, and CSV rows with header \texttt{k,O,sigma}.
...
\begin{enumerate}[label=\textbf{Step \arabic*:}, leftmargin=*]
  ...
  The analysis plan must be fixed before signal thresholds are inspected.
  ...
  Collaborators supply $(k, O, \sigma)$ tuples; they need not adopt CH theory to produce them.
  ...
  Without a flat control, a threshold in $\alpha(k)$ is inconclusive (Table~\ref{tab:verdict}).

  \item \textbf{Analyze} with \py{control_channel_analysis.py} (flat vs threshold on signal; flat mandatory on control).
  The script returns a discrete verdict so interpretation is not negotiated after the fact.

  \item \textbf{Optional follow-up:} if a threshold is detected, run \py{gradient_threshold_analysis.py --joint} when Tier~B adds $\Delta V$;\\
  then the four-step $\xi$ route in Section~\ref{sec:xi-lab-route} (\py{ch_xi_lab_bridge.py}).
  Turn-on gap $d_c$ is the immediate laboratory output; a unique $\xi$ additionally requires ripple period $a_{\mathrm{vac}}$ from Prediction~\#3 and the cross-check $d_c \approx a_{\mathrm{vac}}/(2\pi)$.
\end{enumerate}
...
A ripple ``detection'' at a single gap is insufficient: CH predicts correlated turn-on across the knob scan, while QFT predicts $O(k)$ flat within noise.
Nested flat and threshold models with a fixed $\Delta\chi^2$ cut make that distinction explicit before unblinding.
\begin{align}
...
\textbf{Joint fit:} when both $\alpha$ and $\Delta V$ are available, refit with a \textbf{shared} $k_c$ and separate amplitudes per channel (\py{gradient_threshold_analysis.py --joint}).
...
Full printable checklist: Section~\ref{sec:protocol} and Appendix~\ref{app:collab} (repository scripts: \texttt{control\_channel\_analysis.py})~\cite{CHSpaceFabric2026}.

\subsection{Systematics and limitations}
\label{sec:systematics}

CH is untested; any threshold in $\alpha(k)$ will prompt the question whether gradient gating is required or whether Casimir systematics suffice.
We list known risks qualitatively here so they are on the record before data arrive; the registered flat control channel remains the primary guard, not post-hoc cuts.
...
Table~\ref{tab:verdict} is the operational definition of ``consistent with CH'' for Prediction~\#7.
A single significant fit on one channel would not establish a gradient-gated sector; the outcomes below require threshold signal, flat control, and---when both are measured---consistent $k_c$ across channels.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.42\linewidth}p{0.48\linewidth}@{}}
    ...
    Insufficient signal or failed control & Inconclusive (not CH falsification at grain-scale amplitude) \\
    \bottomrule
  \end{tabular}
\end{table}
...
On demo data the script reports $\Delta\chi^2 \approx 191$ ($\alpha$), $\Delta\chi^2 \approx 244$ ($\Delta V$), control $\Delta\chi^2 \approx 1.3$, joint $\Delta\chi^2 \approx 436$, verdict \texttt{CONSISTENT WITH GRADIENT-GATED CH}~\cite{CHSpaceFabric2026}.
Figure~\ref{fig:protocol} shows the corresponding signal and control fits on that demo set.
...
A positive threshold fit returns a turn-on knob $k_c$, not a first-principles healing length.
The steps below document how laboratory outputs \emph{could} be mapped to $\xi$ and then to emergent-gravity diagnostics if---and only if---real data support the identification hypotheses; they are \textbf{not} part of the Tier~A pass/fail decision.
...
\subsection{Practical lab route to $\xi$}
\label{sec:xi-lab-route}

After a \textbf{positive} Prediction~\#7 verdict (threshold in signal, flat control), the following \textbf{laboratory} sequence fixes $\xi$ in SI units.
It is \textbf{optional} for Tier~A pass/fail (which needs only $k_c$ and gating shape) but is the strongest CH-internal test that turn-on gap and ripple period refer to the \textbf{same} healing length.

\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*]
  \item \textbf{Tier~A Casimir gap scan $\to$ threshold fit $\to$ $k_c$ and $d_c$.}
  Complete the workflow of Section~\ref{sec:protocol} through \py{control_channel_analysis.py}.
  Extract a shared $k_c$ with \py{gradient_threshold_analysis.py --joint} when both $\alpha$ and $\Delta V$ are available.
  For gap scans ($k = 1/d_{\mathrm{nm}}$; Table~\ref{tab:knobs}), the fitted $k_c$ implies turn-on gap
  \begin{equation}
    d_c \approx \frac{1}{k_c}\,\mathrm{nm} = \frac{1}{k_c \times 10^9}\,\mathrm{m}.
    \label{eq:dc-from-kc}
  \end{equation}
  First identification: $\xi \approx d_c$ (Prediction~\#7 $\leftrightarrow$ GPE bulk turn-on).

  \item \textbf{Fit $a_{\mathrm{vac}}$ from $F/F_{\mathrm{Cas}}$ oscillations.}
  On the \textbf{same} setup (preferred) or a dedicated high-statistics ripple run, extract the period $a_{\mathrm{vac}}$ of the oscillatory residual in $F(d)/F_{\mathrm{Cas}}(d)$ (Prediction~\#3, Eq.~\ref{eq:casimir-ripple}; practice: \texttt{casimir\_ripple\_sim.py}).
  Second identification: $\xi = a_{\mathrm{vac}}/(2\pi)$.

  \item \textbf{Cross-check:} $d_c \stackrel{?}{\approx} a_{\mathrm{vac}}/(2\pi)$.
  Equivalently, the two $\xi$ estimates ($\xi \approx d_c$ and $\xi = a_{\mathrm{vac}}/(2\pi)$) should agree within the pre-registered factor of 2 used elsewhere for $k_c$ consistency.
  Agreement supports one healing length; disagreement flags wrong identification, systematics, or need for refined templates---not a laptop fix.

  \item \textbf{Run \py{ch_xi_lab_bridge.py}} on lab CSVs with measured $a_{\mathrm{vac}}$ (command below).
  The script returns consensus $\xi$ (geometric mean when both IDs agree), dispersion/$\beta_{\max}$ consistency checks, and an optional gravity v3 forecast at grain $\alpha_G^{\mathrm{hydro,ref}}$ (Step~3 below).
  Demo: \py{ch_xi_lab_bridge.py --demo}; end-to-end: \py{ch_lab_pipeline_demo.py}.
\end{enumerate}

\noindent\textbf{Example \py{ch_xi_lab_bridge.py} command} (replace paths with lab exports):
\begin{verbatim}
python3 ch_xi_lab_bridge.py \
  --csv-alpha lab/run001_alpha.csv \
  --csv-vis   lab/run001_vis.csv \
  --a-vac 1.50e-7
\end{verbatim}

\textbf{Step 1 --- extract $k_c$ (detail).}
After threshold detection, the joint fit (\py{gradient_threshold_analysis.py --joint}) returns a shared $k_c$ with per-channel amplitudes; Eq.~\ref{eq:dc-from-kc} maps it to $d_c$.

\textbf{Step 2 --- $\xi$ hypotheses (detail).}
Two mechanisms are cross-checked in \texttt{ch\_xi\_prediction.py} and \texttt{ch\_xi\_lab\_bridge.py}:
\begin{itemize}[leftmargin=*]
...
\textbf{Step 3 --- gravity forecast and grain $\alpha_G$ (laptop sketch).}
Spherical defect solves (\texttt{ch\_gpe\_gravity.py}, solver \texttt{v3\_sm\_matched}) use a matter sink $S_M$ on an inner domain and a Schwarzschild-matched exterior at $r_{\mathrm{join}}$.
Exterior Madelung $Q$ is negligible for $r \geq r_{\mathrm{join}}$; the hydrostatic $\delta\rho$ channel dominates.
The hydrostatic coupling has a natural grain-scale reference (not a Newton fit):
\begin{equation}
  \alpha_G^{\mathrm{hydro,ref}} = \frac{m_{\mathrm{grain}}\, c^2}{2 c_s^2}
  \label{eq:alpha-hydro-ref}
\end{equation}
(with $c_s=c$, $\alpha_G^{\mathrm{hydro,ref}} = m_{\mathrm{grain}}/2$), fixed by the same $(\xi, c_s, m_{\mathrm{grain}})$ identities as Eq.~\ref{eq:grain}.
Far-field Newton factors must be evaluated on $r \geq r_{\mathrm{join}}$ only; interior join gradients can falsely imply $N_{\mathrm{hydro}} \sim 10^2$ overshoot if the full grid is used.
With exterior evaluation, the v3 matched solve gives $N_{\mathrm{hydro}}(\alpha_G^{\mathrm{hydro,ref}}) \approx 0.98$ at $\xi = \SI{50}{nm}$ (\texttt{ch\_alpha\_g\_grain\_estimate.py})---the primary laptop result.
An optional profile consistency trim $f = 1/N_{\mathrm{hydro}}(\alpha_G^{\mathrm{hydro,ref}})$ brings $|a|/(GM/r^2)$ to unity in the fit band when needed ($f \approx 1.02$; \texttt{ch\_alpha\_g\_profile\_correction.py}).
Full $\mathcal{L}_{\mathrm{int,grav}}$, dynamical $\Phi$, and metric routes are deferred to companion note \texttt{gravity-sketches.tex} in the same repository~\cite{CHSpaceFabric2026}.

\textbf{End-to-end pipeline.}
\texttt{ch\_lab\_pipeline\_demo.py} runs protocol verdict $\to$ $k_c$ $\to$ $\xi$ $\to$ gravity v3 $\to$ $\alpha_G^{\mathrm{hydro,ref}}$ (optional $f$-trim) on demo CSVs.
On synthetic demo data ($k_c \approx 10~\mathrm{nm}^{-1}$, joint $\Delta\chi^2 \approx 436$), gap turn-on gives $\xi \approx 0.1$~nm and $N_{\mathrm{Newton}} \approx 1.0$~\cite{CHSpaceFabric2026}.
...
Every number quoted in this paper is produced by a named script in the open GitHub repository\\
\url{https://github.com/G-Web-Essentials/CH-A-Gross-Pitaevskii-Superfluid-Vacuum}~\cite{CHSpaceFabric2026}.
The order below runs from the completed GRB reanalysis through GPE forecasts, the protocol demo, and the power study.

\textbf{Recommended reproduction order}:
\begin{enumerate}[label=\arabic*., leftmargin=*]
  \item \py{fermi_grb090510_lat_extended_beta_limit.py}\\
    Download/query LAT extended events, fit $\beta$, reproduce Fig.~\ref{fig:grb}.
  \item \py{ch_gpe_casimir_gap.py} --- wall vs bulk $\chi$ vs $d$ at $\xi = \SI{50}{nm}$ (Fig.~\ref{fig:gpe}).
  \item \py{ch_gpe_to_threshold_demo.py} --- GPE $\to$ CSV $\to$ $\Delta\chi^2$ for gap knob.
  \item \py{control_channel_analysis.py --demo}\\
    Full protocol verdict (Fig.~\ref{fig:protocol}).
  \item \py{ch_lab_pipeline_demo.py} --- $k_c \to \xi$ $\to$ gravity v3 sketch (Section~\ref{sec:lab-xi-bridge}).
  \item \py{ch_threshold_power_study.py} --- Monte Carlo 90\% power vs $\alpha_{\max}$, $n_k$, $\sigma_\alpha$ (Section~\ref{sec:power-study}).
\end{enumerate}

\textbf{Protocol and analysis scripts} (collaborators need only the first four for Tier~A):
\begin{itemize}[leftmargin=*]
  \item \py{gradient_threshold_analysis.py} --- flat vs threshold fits; \texttt{--joint} for shared $k_c$;
  \item \py{control_channel_analysis.py} --- signal + control verdict;
  \item \py{ch_gpe_sphere_to_threshold_demo.py} --- curvature $R$ pipeline;
  \item \py{ch_xi_lab_bridge.py} --- protocol CSVs $\to$ $\xi$ hypotheses;
  \item \py{casimir_ripple_sim.py} --- practice ripple extraction from $F/F_{\mathrm{Cas}}$.
\end{itemize}
Gravity numerics beyond the Step~3 sketch live in \texttt{gravity-sketches.tex} and the repository; they are not required to run the Prediction~\#7 protocol.
...
If the threshold test succeeds, what comes next?
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Prediction~\#3 + \#7:} Casimir ripple period $a_{\mathrm{vac}}$ (Eq.~\ref{eq:casimir-ripple}) and threshold $k_c$ should map to the \textbf{same} $\xi$ via the four-step lab route of Section~\ref{sec:xi-lab-route} ($\xi \approx d_c$, $\xi = a_{\mathrm{vac}}/(2\pi)$, cross-check $d_c \approx a_{\mathrm{vac}}/(2\pi)$).
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Mass--deficit check:} distinguish $M_{\mathrm{grav}}$ (from $r_s$) from $M_{\mathrm{inner}}$ (inner integral); compare microphysical $\gamma_0^{\mathrm{ref}} = 2\lambda_v$ prediction (\texttt{ch\_defect\_microphysics.py}) to an independent target once $\xi$ is lab-fixed (\texttt{ch\_mass\_closure\_link.py}).
...
\textbf{Kinematic time dilation (sketch):} time and motion are coupled in the same $\psi$: a moving defect pattern advances phase both in time ($\partial S/\partial t$, clock ticks) and along its path ($|\nabla S|$).
Faster motion reallocates that single phase budget; low-energy modes then see the acoustic metric of a flowing superfluid, with lapse $\propto (c_s^2 - v^2)$ when $c_s \to c$ (analog-gravity program~\cite{Unruh1981,Visser1995}).
This motivates emergent special-relativity clock slowing without a separate Minkowski postulate; it is \textbf{not} derived here to the level of Prediction~\#7 (companion detail: \texttt{universal-laws.tex}).
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Laptop consistency (done, not proof):} \texttt{ch\_clock\_redshift\_from\_gpe.py} (with $\alpha_G^{\mathrm{hydro,ref}}$ and exterior evaluation) compares $\omega \propto \sqrt{\rho}$ and $\Phi_{\mathrm{CH}}$ clock proxies to Schwarzschild GR on the analytic tail and v3 exterior (Fig.~\ref{fig:clock-gr}); \texttt{ch\_acoustic\_light\_bending.py} excludes naive $n \propto 1/\sqrt{\rho}$ lensing---see Section~\ref{sec:numerics-consistency} and \texttt{universal-laws.tex}.
\end{itemize}
These are longer-horizon than Tier~A Casimir but are the path from ``gated ripple'' to ``time is hydrodynamic.''

\begin{figure}[htbp]
  \centering
  \maybeincludegraphics[width=0.85\linewidth]{figures/ch_clock_redshift_vs_gr.png}
  \caption{Clock redshift $|z|$ vs $r/r_s$: analytic $(1-r_s/r)^2$ tail and v3 exterior with profile-rule $\alpha_G$ ($\omega \propto \sqrt{\rho}$), compared to Schwarzschild GR. Same $\alpha_G$ as the gravity pipeline; no separate clock fit (\texttt{ch\_clock\_redshift\_from\_gpe.py}, \texttt{--gravity}).}
  \label{fig:clock-gr}
\end{figure}
\FloatBarrier
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Roadmap from Prediction~\#7 toward the full CH framework.}
  \label{tab:roadmap}
...
Each stage publishes a \textbf{narrower} claim than the full ontology; that discipline keeps the program falsifiable and legible to outside collaborators.
...
\noindent\textbf{Supported (qualitatively):} defect Newton matching; attractive lensing via $n(\Phi)$ or $n \approx 1 - r_s/r$; large-$n$ Kepler correspondence; \textbf{sphere--plate curvature:} axisymmetric GP (\texttt{slice1d} mode, $\hat{R}\gtrsim 3$) matches Thomas--Fermi on the radial rim probe to median $\sim$18\%, max $\sim$23\% over $R \in [1,100]~\mu\mathrm{m}$ at $d_{\min}=100$~nm, $\xi=50$~nm (Fig.~\ref{fig:sphere-plate-gp-overlay}).
...
\begin{enumerate}[label=\arabic*., leftmargin=*]
  \item \textbf{EM--GPE derivation of the Casimir ripple} --- \textbf{partially} addressed in companion Paper~2~\cite{CHPaper2} (gradient gating and period motivated; Eq.~\ref{eq:casimir-ripple} remains parameterized here for the protocol).
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.14\linewidth}p{0.80\linewidth}@{}}
    ...
    Methodological &
    Knob definitions, flat vs threshold fits, $\Delta\chi^2 \geq 9$ cut, $k_c$ agreement (factor 2), and mandatory flat control are pre-registerable. Outcomes rule the \textbf{gating shape} in or out in the scanned range; grain-scale amplitude may yield inconclusive without falsifying CH (Section~\ref{sec:power-study}). \\
    Physical &
...
\textbf{What is not established:} no positive CH signal in real data; $\xi$ not fixed from first principles; geometry-specific $k \to |\nabla\rho|$ maps remain open; exterior Newton at $\mathcal{O}(1)$ from $\alpha_G^{\mathrm{hydro,ref}}$ is a laptop sketch only---full defect Lagrangian and 3D $S_M$ remain open (\texttt{gravity-sketches.tex}); laptop Layer~2--3 checks (Table~\ref{tab:numerics-consistency}) are consistency sketches only.
...
\textbf{Gating shape vs amplitude.}
Prediction~\#7 is primarily a \textbf{model comparison on $\alpha(k)$ shape}: flat (QFT) vs threshold turn-on (CH) when $\chi_{\mathrm{bulk}}$ rises with the knob.
The Monte Carlo uses $\alpha_{\mathrm{eff}}(k) = \alpha_{\max}\chi_{\mathrm{bulk}}(d)$; at large $\alpha_{\max}$ the fit has high power to detect that \textbf{pattern}.
If the true coupling is at the grain-scale band ($\alpha_{\max} \sim 0.1$--$0.2\%$; Paper~2~\cite{CHPaper2}), an \textbf{inconclusive} amplitude verdict under the moderate $\sigma_\alpha$ defaults is expected---that is \textbf{not} a CH falsification, provided controls stay flat and no spurious turn-on appears.
Collaborators should still register pilot $\sigma_\alpha$ and re-run the power study; a null on \textbf{gating shape} (flat $\alpha(k)$ with adequate $\chi$ swing) would rule out the gated sector in the scanned range.

\begin{table}[htbp]
  \centering
  \small
  \caption{Monte Carlo threshold-detection power (\texttt{ch\_threshold\_power\_study.py}, $\xi = \SI{50}{nm}$, $d = \SI{40}{nm}$--\SI{600}{nm}, $n_k = 12$, $n_{\mathrm{repeat}} = 20$, $n_{\mathrm{trials}} = 400$, bulk coupling).}
...
Table~\ref{tab:gap-scan} lists a representative GPE-recommended scan; Figure~\ref{fig:power-study} shows how detection power scales with $\alpha_{\max}$ and $n_k$ at the planning defaults of Section~\ref{sec:forecast-defaults}.
...
Companion Paper~2~\cite{CHPaper2} tightens microphysics priors ($\alpha_{\max}$, $a_{\mathrm{vac}}$) without altering the Prediction~\#7 decision rules in this manuscript.
...
\noindent Zenodo preprint (v1):\\
\url{https://doi.org/10.5281/zenodo.21296947}.

\noindent Code and protocol scripts:\\
\url{https://github.com/G-Web-Essentials/CH-A-Gross-Pitaevskii-Superfluid-Vacuum}~\cite{CHSpaceFabric2026}.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}X@{}}
    ...
    Symbol & Meaning \\
    \midrule
    $\psi$ & Vacuum order parameter (complex BEC field) \\
    $\rho$ & Vacuum condensate density; $\rho = |\psi|^2$ (Eq.~\ref{eq:madelung}) \\
    $S$ & Madelung phase; $\psi = \sqrt{\rho}\,e^{iS/\hbar}$ \\
    $\mathbf{v}$ & Superfluid velocity; $\mathbf{v} = \nabla S/\rho$ (Eq.~\ref{eq:madelung}) \\
    $|\nabla\rho|$ & Magnitude of the spatial gradient of $\rho$; large near boundaries/defects, $\approx 0$ in uniform void \\
    \midrule
    ...
    $c_s$ & Condensate sound speed; $c$ in the low-energy limit (Eq.~\ref{eq:grain}) \\
    $G$ & Emergent Newton constant; $G = \alpha_G c_s^2 \xi / \rho_{\mathrm{in}}$ (Eq.~\ref{eq:G}) \\
    $\alpha_G$ & Dimensionless gravity coupling in Eq.~\ref{eq:G}; often unity in sketches \\
    \midrule
    $|\nabla\rho|_c$ & $\rho_{\mathrm{in}}/\xi \approx c^2/G$; gradient-gate turn-on scale \\
    ...
    \midrule
    $\alpha$ & Casimir ripple amplitude from $F/F_{\mathrm{Cas}}$ oscillatory fit (Prediction~\#3) \\
    $\alpha_{\max}$, $\Delta V_{\max}$ & Gated observable ceilings before $\chi$; $\alpha_{\max}=0.12$ is an upper sensitivity default\\
    & (preferred $\sim 0.1$--$0.2\%$ in Paper~2~\cite{CHPaper2}) \\
    $\alpha_{\mathrm{eff}}$, $\Delta V$ & $\alpha_{\max}\chi$, $\Delta V_{\max}\chi$ \\
    $\beta_{\max}$ & $(3/2)\xi^2/\hbar^2$; dispersion ceiling (Prediction~\#4) \\
    $\beta_{\mathrm{eff}}$ & $\beta_{\max}\chi$; used in GRB fits \\
    $a_{\mathrm{vac}}$ & Ripple period in Eq.~\ref{eq:casimir-ripple}; lattice-scale fit parameter \\
    \midrule
    $d$, $d_c$ & Casimir plate gap (signal knob); turn-on gap $d_c \approx 1/k_c$~nm (Eq.~\ref{eq:dc-from-kc}) \\
    $R$ & Sphere radius (curvature knob); $k = 1/R_{\mu\mathrm{m}}$ \\
    $k$ & Dimensionless experimental knob; $1/d_{\mathrm{nm}}$ or $1/R_{\mu\mathrm{m}}$ \\
    $k_c$ & Threshold knob value in Eq.~\ref{eq:threshold-fit} \\
    $O(k)$ & Scalar observable at knob $k$ ($\alpha$ or $\Delta V$ after extraction) \\
    $\sigma$ & Uncertainty on $O$ at each $k$ (CSV column \texttt{sigma}) \\
    $c_0$, $A$ & Flat baseline and threshold amplitude in Eq.~\ref{eq:threshold-fit} \\
    $\Delta\chi^2$ & $\chi^2_{\mathrm{flat}} - \chi^2_{\mathrm{thr}}$; detection if $\geq 9$ \\
  \midrule
...
\textbf{Offering:} Pre-registered analysis (\py{control_channel_analysis.py}), open repository, co-authorship.
