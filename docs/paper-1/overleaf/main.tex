...
\begin{enumerate}[label=\textbf{Step \arabic*:}, leftmargin=*]
  ...
  \item \textbf{Optional follow-up:} if a threshold is detected, run \texttt{gradient\_threshold\_analysis.py --joint} when Tier~B adds $\Delta V$, then the four-step $\xi$ route in Section~\ref{sec:xi-lab-route} (\texttt{ch\_xi\_lab\_bridge.py}).
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*]
  ...
  Complete the workflow of Section~\ref{sec:protocol} through \texttt{control\_channel\_analysis.py}.
  Extract a shared $k_c$ with \texttt{gradient\_threshold\_analysis.py --joint} when both $\alpha$ and $\Delta V$ are available.
  ...
  \item \textbf{Run \texttt{ch\_xi\_lab\_bridge.py}} on lab CSVs with measured $a_{\mathrm{vac}}$ (command below).
  ...
  Demo: \texttt{ch\_xi\_lab\_bridge.py --demo}; end-to-end: \texttt{ch\_lab\_pipeline\_demo.py}.
\end{enumerate}

\noindent\textbf{Example \texttt{ch\_xi\_lab\_bridge.py} command} (replace paths with lab exports):
\begin{verbatim}
...
After threshold detection, the joint fit (\texttt{gradient\_threshold\_analysis.py --joint}) returns a shared $k_c$ with per-channel amplitudes; Eq.~\ref{eq:dc-from-kc} maps it to $d_c$.
...
Every number quoted in this paper is produced by a named script in the open GitHub repository \url{https://github.com/G-Web-Essentials/CH-A-Gross-Pitaevskii-Superfluid-Vacuum}~\cite{CHSpaceFabric2026}.
...
\begin{enumerate}[label=\arabic*., leftmargin=*]
  ...
  \item \texttt{ch\_lab\_pipeline\_demo.py} --- $k_c \to \xi$ $\to$ gravity v3 sketch (Section~\ref{sec:lab-xi-bridge}).
...
\begin{itemize}[leftmargin=*]
  \item \texttt{gradient\_threshold\_analysis.py} --- flat vs threshold fits; \texttt{--joint} for shared $k_c$;
  ...
  \item \texttt{ch\_xi\_lab\_bridge.py} --- protocol CSVs $\to$ $\xi$ hypotheses;
  \item \texttt{casimir\_ripple\_sim.py} --- practice ripple extraction from $F/F_{\mathrm{Cas}}$.
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\alpha_{\max}$, $\Delta V_{\max}$ & Gated observable ceilings before $\chi$; $\alpha_{\max}=0.12$ is an upper sensitivity default (preferred $\sim 0.1$--$0.2\%$ in Paper~2~\cite{CHPaper2}) \\
