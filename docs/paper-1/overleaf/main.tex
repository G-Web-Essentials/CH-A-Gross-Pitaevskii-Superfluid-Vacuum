...
Companion Paper~2~\cite{CHPaper2} (in preparation) derives gradient gating in a perturbative EM--GPE bridge and favours $\alpha_{\max} \sim 0.1$--$0.2\%$ from grain microphysics; the $12\%$ and $\kappa=0.12$ defaults here remain upper sensitivity cases for the power study.
\end{abstract}
...
\textbf{Companion Paper 2 (in preparation).}
A separate manuscript~\cite{CHPaper2} couples GPE boundary profiles to a perturbative Casimir mode sum and \textbf{motivates gradient gating} ($\delta F/F_{\mathrm{Cas}} \propto \chi_{\mathrm{bulk}}$ with corr $\approx 0.94$ at $\xi = \SI{50}{nm}$).
...
Full $\mathcal{L}_{\mathrm{int,grav}}$, dynamical $\Phi$, and metric routes are deferred to companion note \texttt{gravity-sketches.tex} and \texttt{docs/ch-gravity-roadmap-g7-g11.md}~\cite{CHSpaceFabric2026}.
